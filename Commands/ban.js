...
module.exports = {
    ...
    async execute(message, args) {
        if (!message.member.hasPermission("BAN_MEMBERS")) return message.reply("No tienes permiso para usar este comando")
        let User = await message.mentions.members.first() || message.guild.members.cache.get(args[0]) || message.guild.members.cache.find(r => r.user.username.toLowerCase().includes() === args.join(' ').toLocaleLowerCase()) || message.guild.members.cache.find(r => r.displayName.toLowerCase().includes() === args.join(' ').toLocaleLowerCase())
             || message.member;let banReason = args.join(" ").slice(23)
        if (!banReason) {
            banReason = "No se proporcionó ninguna razón"
        }
        ...
        let noUser = new Discord.MessageEmbed()
                ...
                .setColor(colors.red)
                .setDescription('Por favor proporcione un usuario válido')
        ...
        let banDmEmbed = new Discord.MessageEmbed()
            ...
            .setColor(colors.red)
            .setDescription(`**Te han baneado de: \`${message.guild.name}\`**\n**Razon:** ${banReason}\n**Staff:** <@${message.author.id}> (${message.author.id})`)
            .setTimestamp()
            .setFooter(message.client.user.username, message.client.user.avatarURL({
                dynamic: true
            }))
